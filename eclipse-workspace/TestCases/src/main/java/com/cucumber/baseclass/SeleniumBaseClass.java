...
public class SeleniumBaseClass {
	...
	public void pageTitleRelevance(String text) throws Exception {
		if(driver.getTitle().contains(text)) {
			System.out.println("Page title relevant to "+text);
		}else {
			throw new Exception("Page title not in refernce to "+text);
		}
	...
	public static void inputValuesInElement(WebElement element, String value) throws Exception {
		try {
			element.clear();
			element.sendKeys(value);
		} catch (Exception e) {
			Reporter.addStepLog(value + " couldnt be entered");
			throw new Exception(value + " couldnt be entered");
		}
	...
	public static File takeScreenShot(String filename) throws Throwable {
		...
		try {
			...
			File des = new File(System.getProperty("user.dir")+
					"\\src\\test\\resource\\com\\cucumber\\reports\\Screenshot\\" + filename + ".png");
